    ...
    fn seek(&mut self, pos: std::io::SeekFrom) -> std::io::Result<u64> {
        match pos {
            ...
            std::io::SeekFrom::End(offset) => {
                ...
                // Check that file_size is within i64 range
                if file_size < 0.0 || file_size > i64::MAX as f64 {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidInput,
                        format!("File size {file_size} is out of range for i64"),
                    ));
                }
                let file_size_i64 = file_size as i64;
                let new_pos = file_size_i64.checked_add(offset).ok_or_else(|| {
                    std::io::Error::new(
                        std::io::ErrorKind::InvalidInput,
                        "Seek resulted in an out-of-range file position",
                    )
                })?;
                if new_pos < 0 {
                    return Err(std::io::Error::new(
